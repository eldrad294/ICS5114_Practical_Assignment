...
# Kafka topic which this produces will subscribe to
kafka_topic = g_config.get_value('ConsumerRunner', 'video')

# Kafka consumer group name for balanced consumers
kafka_consumer_group = g_config.get_value('ConsumerRunner', 'kafka_consumer_group')
...
bconsumer = consumer.set_balanced_consumer(topic=kafka_topic, consumer_group=kafka_consumer_group,
                                           zookeeper_connect=zookeeper_connection, auto_commit_enable=True)
